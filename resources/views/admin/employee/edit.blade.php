...
@section('content')
    <div class="container">
        ...
        @if (!empty($data['account']))
            <p>Đã tạo tài khoản</p>
        @else
            <a class="btn btn-success" href="{{ route('admin.account.create', ['employee' => $data['id']]) }}"
                class="btn btn-default">
                <i class="fa-solid fa-user"></i>
                Thêm tài khoản
            </a>
        @endif
        <form class="row" action="{{ route('admin.employee.update') }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('put')
                        ...
                        <select id="" name="position">
                            ...
                            @forelse ($positions as $item)
                                <option value="{{ $item['id'] }}"
                                    {{ $data['position'] == $item['id'] ? 'selected' : '' }}>
                                    {{ $item['name'] }}</option>
                            @empty
                            @endforelse
                        </select>
                    ...
                    <div class="checkout__input">
                        ...
                        <select name="department">
                            ...
                            @forelse ($departments as $item)
                                <option value="{{ $item['id'] }}"
                                    {{ $data['department'] == $item['id'] ? 'selected' : '' }}>
                                    {{ $item['name'] }}</option>
                            @empty
                            @endforelse
                        </select>
                ...
                <div class="col-4">
                    <div class="checkout__input">
                        ...
                        <select name="manager_department">
                            ...
                            @if (!empty($data['manager_department']))
                                <option value="{{ $data['manager_department']['id'] }}" selected>
                                    {{ $data['manager_department']['name'] }}</option>
                            @endif
                            @forelse ($departments as $item)
